...
const EditProductType = ({ product_typeId, onCancel, onSuccess }) => {
  ...
  const fetchProductTypeData = async (id) => {
    ...
    try {
      const allProductTypes = await fetchDataList('https://dx.hoangphucthanh.vn:3000/maintenance/product-types');
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const response = await updateItemById(`https://dx.hoangphucthanh.vn:3000/maintenance/product-types/${product_typeId}`, payload);
  ...
  return (
    <div className="edit-container">
      <Card
        title={`Chỉnh sửa Loại Hàng: ${product_typeData?.ten_loai_hang || product_typeId}`}
        bordered={false}
        className="edit-card"
      >
        {fetchLoading ? (
